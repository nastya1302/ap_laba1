...
def get_images(name: str, num_img: int) -> List[str]:
    ...
    elem1:str = driver.find_element(By.CLASS_NAME, "MMImage-Origin").get_attribute('src')
    links:List[str] = [elem1]
    for _ in range(num_img):
        try:
            sleep(1)
            driver.find_element(By.CLASS_NAME, "MediaViewer_theme_fiji-ButtonNext").click()                
            link:str = driver.find_element(By.CLASS_NAME, "MMImage-Origin").get_attribute('src')
            links.append(link)
        except:
            continue
    driver.close()
